"""
FeudalAgentV5 — HRL FeUdal v5 (05d_5 / 05s_5, 1-phase training).

Implementació autònoma extreta del notebook 05d_5_hrl_FeUdal_agent.ipynb.

PerceptionModule: obs → hidden (Tanh) → z_dim (Linear, sense activació final)

Usage:
    from src.rl.agents.feudal_agent_v5 import FeudalAgentV5
    agent = FeudalAgentV5.load("checkpoints_dense/hrl_dense_feudal_agent_paper_1phase/hrl_feudal_best.pt")
...
class PerceptionModule(nn.Module):
    """Percepció compartida: obs_dim → hidden (Tanh) → z_dim (Linear, sense activació)."""
...
class FeudalAgentV5(nn.Module):
    """Agent HRL FeUdal v5 (1-phase training, PerceptionModule linear output)."""
    ...
    @classmethod
    def load(cls, path, device="cpu") -> "FeudalAgentV5":
