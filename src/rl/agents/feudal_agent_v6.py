"""
FeudalAgentV6 — HRL FeUdal v6 (05d_6 / 05s_6, 1-phase + pretrain).

Implementació autònoma extreta del notebook 05d_6_hrl_FeUdal_agent.ipynb.

PerceptionModule: obs → hidden (Tanh) → z_dim (Tanh) — Tanh en TOTES les capes.

Usage:
    from src.rl.agents.feudal_agent_v6 import FeudalAgentV6
    agent = FeudalAgentV6.load("checkpoints_dense/hrl_dense_feudal_agent_paper_1ph_pt/hrl_feudal_best.pt")
...
class PerceptionModule(nn.Module):
    """Percepció compartida: obs_dim → hidden (Tanh) → z_dim (Tanh).

    Diferència v6: Tanh en TOTES les capes incloent la sortida z.
    """
...
class FeudalAgentV6(nn.Module):
    """Agent HRL FeUdal v6 (1-phase + pretrain, PerceptionModule Tanh en totes les capes)."""
    ...
    @classmethod
    def load(cls, path, device="cpu") -> "FeudalAgentV6":
